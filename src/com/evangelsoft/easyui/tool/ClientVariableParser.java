...
import com.evangelsoft.workbench.security.intf.SysUserPa;

/**
 * 解析数据类型
 * */
public class ClientVariableParser
{
	...
	protected static Object getObject(String s, String s1)
	{
		...
		if (s.equalsIgnoreCase("PRIV_UNIT_LIST"))
			...
			{
				String s3 = s1;
				SysUserPa sysuserpahome = (SysUserPa)(new RMIProxy(Consumer.getDefaultConsumer().getSession())).newStub(SysUserPa.class);

				Object aobj[] = {
						Consumer.getDefaultConsumer().getSession().getContext().getTopic("USER_ID"), Consumer.getDefaultConsumer().getSession().getContext().getTopic("OWNER_ID")
				};
			...
			s2 = ((BigDecimal)Consumer.getDefaultConsumer().getEnv("OWNER_ID"));
		else
			if (s.equals("OWNER_NAME"))
				s2 = ((String)Consumer.getDefaultConsumer().getEnv("OWNER_NAME")).toString();
		else
		if (s.equals("USER_ID"))
			s2 = ((BigDecimal)Consumer.getDefaultConsumer().getEnv("USER_ID"));
		else
			if (s.equals("USER_NAME"))
				s2 = ((String)Consumer.getDefaultConsumer().getEnv("USER_NAME")).toString();
		else
...
//			s2 = simpledateformat8.format(calendar5.getTime());
				...
				try
				{
					...
					Object aobj[] = {
						Consumer.getDefaultConsumer().getEnv("USER_ID"), Consumer.getDefaultConsumer().getEnv("OWNER_ID")
					};
					VariantHolder<String> variantholder = new VariantHolder<String>();
					VariantHolder<Boolean> variantholder1 = new VariantHolder<Boolean>();
					VariantHolder<Object> variantholder2 = new VariantHolder<Object>();
		...
		//默认当前组织编号
			...
			{
				s2 = Consumer.getDefaultConsumer().getEnv("OWNER_CODE").toString();
			}
	...
	protected static String getString(String s, String s1)
	{
		String s2=null;
		if (s.equalsIgnoreCase("PRIV_UNIT_LIST"))
			try
		...
		if (s.equals("OWNER_ID"))
			s2 = ((BigDecimal)Consumer.getDefaultConsumer().getEnv("OWNER_ID")).toString();
		else
			if (s.equals("OWNER_NAME"))
				s2 = ((String)Consumer.getDefaultConsumer().getEnv("OWNER_NAME")).toString();
		else
		if (s.equals("USER_ID"))
			s2 = ((BigDecimal)Consumer.getDefaultConsumer().getEnv("USER_ID")).toString();
		else
			if (s.equals("USER_NAME"))
				s2 = ((String)Consumer.getDefaultConsumer().getEnv("USER_NAME")).toString();
		else
