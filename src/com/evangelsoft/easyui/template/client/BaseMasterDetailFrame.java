...
public class BaseMasterDetailFrame extends UMasterDetailFrame implements EditListener{
	 ...
	 * 打印接口预留
	 */
	HashMap<String, PrintStorageDataSet> printDataSetPanelMap=new HashMap<String, PrintStorageDataSet>();
	...
	//	public void run(String  templateId,String queryId){
	//		this.templateId=templateId;
	...
	public BaseMasterDetailFrame(){
		try{
			...
			//指定当分隔窗格的大小改变时如何分配额外空间。在默认情况下，值为 0 表示右边/底部的组件获得所有额外空间（左边/顶部的组件固定），而值为 1 表示左边/顶部的组件获得所有额外空间（右边/底部的组件固定）。
...
//					middlelowerPane.setDividerLocation(200);
	...
	protected void prepared(Object arg0) throws Exception {
		...
		data.value=new RecordSet[]{new RecordSet(),new RecordSet(),new RecordSet(),new RecordSet(),new RecordSet()};
		...
		if(datas[2]!=null&&datas[2].recordCount()>0){
			for(int i=0;i<datas[2].recordCount();i++){
				...
				if(panle!=null){
					...
					printSet.setTableName(datas[2].getRecord(i).getField("TABS_ID").getString());
					printSet.setTableDesc(datas[2].getRecord(i).getField("TABS_NAME").getString());
					printSet.setDataSet(panle.getStorageDataSet());
					this.printDataSetPanelMap.put(datas[2].getRecord(i).getField("TABS_ID").getString(),printSet);
				}
					...
					if(!datas[3].getRecord(i).getField("DEFAULT_VALUE").isNull()){
						...
						if(initValueMap.containsKey(datas[3].getRecord(i).getField("TABLE_ID").getString())){
							initValueMap.get(datas[3].getRecord(i).getField("TABLE_ID").getString()).put(datas[3].getRecord(i).getField("COLUMN_ID").getString()
									, new DefaultValue(datas[3].getRecord(i).getField("DEFAULT_VALUE_SOURCE").getString(), datas[3].getRecord(i).getField("DEFAULT_VALUE").getString(),masterDataSet));
						}else{
		...
		for(StorageDataSetPanel panel:dataSetPanelMap.values()){
			...
			HashMap<DataSetAware, Boolean> localHashMap = (HashMap<DataSetAware, Boolean>)this.detailComponents.get(panel.getStorageDataSet());
...
//				this.middlelowerPane.setDividerSize(1);
	...
	@Override
	protected void validateDetail(DataSet paramDataSet,
			ReadWriteRow paramReadWriteRow, ReadRow paramReadRow)
					throws Exception {
	...
	public void printDesign(){
		//还有修改的按钮需要考虑
		PrintDesignFrame.createPrint( templateId.toString(), printDataSetPanelMap) ;
	}
}
